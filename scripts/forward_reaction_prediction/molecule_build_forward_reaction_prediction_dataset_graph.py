...
The reaction equation has the following format. The molecule representations are extracted from a strong molecule encoder.
```
reactant1.reactant2. ... .reactantN>>product
```
Your task is to predict the SELFIES representation of the product molecule. We provide the molecule representation of the reactants."""
...
def conversation_train(id, instruction, input, output):
    ...
    return {
        ...
        "messages": [
            ...
            {
                "role": ROLE_USER,
                "content": PROMPT_TEMPLATE.format(instruction=instruction, molecules=molecules)
            },
            {
                "role": ROLE_ASSISTANT,
...
def conversation_test(id, instruction, input, output, few_shots: list):
    ...
    if not few_shots:
        ...
        ) + "\n" + PROMPT_TEMPLATE.format(instruction=instruction, molecules=molecules)
    return {
        "id": id,
        "molecules": {"selfies": selfies, "smiles": smiles},
        "ground_truth": output,
...
# python molecule_build_forward_reaction_prediction_dataset_graph.py --reaction_data_path /gpfs/gibbs/pi/gerstein/xt86/bioagent/data/Mol-Instructions/data/Molecule-oriented_Instructions/forward_reaction_prediction.json --out_dir /gpfs/gibbs/pi/gerstein/xt86/bioagent/data/Mol-Instructions/data/Molecule-oriented_Instructions/forward_reaction_prediction_graph