...
def main(args):
    data_files = {
        "train": os.path.join(args.data_dir, f"reagent_prediction_train.json"),
        "test": os.path.join(args.data_dir, "reagent_prediction_test.json"),
    }
    dataset = {
    ...
    def gen(split):
        for id, item in enumerate(dataset[split]):
            reactions, reagents = item["input"], item["output"]
            reactants, products = reactions.split(">>")
            reactants = sf.decoder(reactants)
            products = sf.decoder(products)
            reagents = sf.decoder(reagents)
            if split == "train":
                result = conversation_train(id, reactants, products, reagents, format=args.format, token=args.token)
            elif split == "test":
                # set num_examples to 0 to disable fs-test
                result = conversation_test(id, reactants, products, reagents,
...
# python molecule_build_reagent_prediction_molins.py --data_dir /cto_labs/AIDD/DATA/Mol-Instructions/Molecule-oriented_Instructions --out_dir /cto_labs/AIDD/DATA/React/MolInstruct/reagent_mmchat_smiles --num_proc 4