...
def main(args):
    data_files = {
        ...
        "test": os.path.join(args.data_dir, "test", "reagent.json"),
    }
    dataset = {
    ...
    def gen(split):
        for id, item in enumerate(dataset[split]):
            reactions, reagents = item["canonical_rxn"], item["reagent"]
            ...
            if split == "train":
                ...
                result = conversation_test(id, reactants, products, reagents, format=args.format, token=args.token)
            elif split == "test":
...
# python molecule_build_reagent_prediction.py --data_dir /cto_labs/AIDD/DATA/React/TextReact/RCR/MolIns --out_dir /cto_labs/AIDD/DATA/React/TextReact/RCR/MolIns/rp_mmchat_smiles --num_proc 4