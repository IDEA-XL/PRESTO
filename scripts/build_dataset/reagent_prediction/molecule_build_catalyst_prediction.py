...
SYSTEM_PROMPT = """You are a chemist. Now you are given a reaction equation. Please predict the possible catalyst of the reaction. The reaction equation has the following format:
```
reactant1.reactant2. ... .reactantN>>product
```
Your task is to predict the <REP_1> representation of the catalyst. We provide the <REP_2> of the reactions."""
...
PROMPT_TEMPLATES = [
    {
        "input": "<MOLECULE> Based on the given chemical reaction, can you propose some likely catalysts that might have been utilized?",
        "output": "A possible catalyst can be <OUTPUT> .",
    },
    {
        "input": "Based on the given chemical reaction <MOLECULE>, suggest some possible catalyst.",
        "output": "The catalyst can be <OUTPUT> .",
    },
    {
        "input": "Can you provide potential catalyst for the following chemical reaction? <MOLECULE>",
        "output": "<OUTPUT> .",
    },
    {
        "input": "Can you suggest some catalysts that might have been used in the given chemical reaction? <MOLECULE>",
        "output": "A probable catalyst could be <OUTPUT> .",
    },
    {
        "input": "<MOLECULE> From the provided chemical reaction, propose some possible catalysts that could have been used.",
        "output": "<OUTPUT> .",
    },
    {
        "input": "Given the following chemical reaction <MOLECULE>, what are some potential catalysts that could have been employed?",
        "output": "<OUTPUT> .",
    },
    {
        "input": "Given the following reaction <MOLECULE>, what are some possible catalysts that could have been utilized?",
        "output": "<OUTPUT> .",
    },
    {
        "input": "Given this chemical reaction <MOLECULE>, what are some catalysts that could have been used?",
        "output": "<OUTPUT> .",
    },
    {
        "input": "<MOLECULE> Please propose potential catalysts that might have been utilized in the provided chemical reaction.",
        "output": "Sure. A potential answer could be: <OUTPUT> .",
    },
    {
        "input": "Please provide possible catalysts based on the following chemical reaction <MOLECULE>.",
        "output": "<OUTPUT> .",
    },
    {
        "input": "Please suggest some possible catalysts that could have been used in the following chemical reaction <MOLECULE>.",
        "output": "<OUTPUT> .",
    },
    {
        "input": "What catalysts could have been utilized in the following chemical reaction? <MOLECULE>",
        "output": "<OUTPUT> .",
    },
]
...
def conversation_test(id, reactants, products, output, few_shots: list = None, format = "smiles", token=True):
    ...
    if not few_shots:
        ...
        few_shot_examples = "\n".join(
            f"Few-shot example {i+1}: reaction:{example['input']}, catalyst:{example['output']}" for i, example in enumerate(few_shots)
        )
...
def main(args):
    data_files = {
        "train": os.path.join(args.data_dir, "train", "catalyst.json"),
        "dev": os.path.join(args.data_dir, "dev", "catalyst.json"),
        "test": os.path.join(args.data_dir, "test", "catalyst.json"),
    }
    dataset = {
    ...
    def gen(split):
        for id, item in enumerate(dataset[split]):
            reactions, catalyst = item["canonical_rxn"], item["catalyst"]
            reactants, products = reactions.split(">>")
            if split == "train":
                result = conversation_train(id, reactants, products, catalyst, format=args.format, token=args.token)
            elif split == "dev":
                result = conversation_test(id, reactants, products, catalyst, format=args.format, token=args.token)
            elif split == "test":
                # set num_examples to 0 to disable fs-test
                result = conversation_test(id, reactants, products, catalyst,
...
# python molecule_build_catalyst_prediction.py --data_dir /cto_labs/AIDD/DATA/React/TextReact/RCR/MolIns --out_dir /cto_labs/AIDD/DATA/React/TextReact/RCR/MolIns/cp_mmchat_smiles --num_proc 4