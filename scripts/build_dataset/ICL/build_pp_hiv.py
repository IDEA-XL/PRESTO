...
SYSTEM_PROMPT = """You are an expert chemist, your task is to predict the property of molecule using your experienced chemical property prediction knowledge. Given the representation of a molecule, the task focuses on predicting molecular properties, specifically inhibit of HIV replication. You will be provided with several examples molecules, each accompanied by a binary label indicating whether a molecule can inhibit (Yes) or cannot inhibit (No) HIV replication. The task is to precisely predict the binary label for a given molecule and its HIV activity test, considering its properties and its potential to impede HIV replication.\n"""
...
n_shot=8
python build_pp_hiv.py --test_csv /cto_labs/AIDD/DATA/ChemLLMBench/property_prediction/HIV_test.csv --train_csv /cto_labs/AIDD/DATA/ChemLLMBench/property_prediction/HIV.csv --out_dir /cto_labs/AIDD/DATA/ChemLLMBench/ICL/pp/hiv_${sample_strategy}_${n_shot}shot_int --n_shot ${n_shot} --sample_strategy ${sample_strategy}
"""